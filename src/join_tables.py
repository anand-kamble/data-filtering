...
files = glob.glob("../TABLES_ADD_20240515/*")
...
for f in files:
    ...
    read_df = pd.read_csv(f, nrows=20, on_bad_lines="skip", encoding="latin-1")
    data[f] = read_df
...
# UNCOMMENT TO PRINT THE COMMON COLUMNS BETWEEN ALL FILES
...
df1 = pd.read_csv("../TABLES_ADD_20240515/ISDP LOGBOOK REPORT.csv", encoding="latin-1")
...
df1["ACTION_DT"] = pd.to_datetime(df1["ACTION_DT"], errors="coerce").astype(int) / 10**9
